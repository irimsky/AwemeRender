...
std::shared_ptr<Mesh> Mesh::fromFile(const std::string& filename)
{
	...
	if (scene && scene->HasMeshes()) {
        // TODO: 这里只加了第一个mesh，一个模型可能有多个mesh
...
std::shared_ptr<Mesh> Mesh::createSphereMesh()
{
    Mesh* mesh = new Mesh();
    if (sphereVAO == 0)
    {
    ...
    mesh->vao = sphereVAO;
    mesh->numElements = indexCount;  
    return std::shared_ptr<Mesh>(mesh);
}

unsigned int Mesh::cubeVAO = 0;
unsigned int Mesh::cubeVBO = 0;
...
unsigned int Mesh::planeVAO = 0;
unsigned int Mesh::planeVBO = 0;
